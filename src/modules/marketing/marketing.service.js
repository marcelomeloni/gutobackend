...
export const create = async (payload) => {
  const { data, error } = await supabase.from('marketing_conteudos').insert([payload]).select().single();
  if (error) throw error;
  return data;
};

export const update = async (id, payload) => {
  const { data, error } = await supabase.from('marketing_conteudos').update(payload).eq('id', id).select().single();
  if (error) throw error;
  return data;
};
